...
class CocoPose(RNGDataFlow):
    ...
    def __init__(self, path, img_path=None, is_train=True, decode_img=True, only_idx=-1):
        ...
        if is_train:
            whole_path = os.path.join(path, 'blouse_train.json')
        else:
            whole_path = os.path.join(path, 'blouse_val.json')
        self.img_path = (img_path if img_path is not None else '') + ('train2018/' if is_train else 'val2018/')
...
if __name__ == '__main__':
    ...
    print("Done 1")
    df = get_dataflow_batch('/Users/yangdongdong/Desktop/dataset/annotations', True, 2, '/Users/yangdongdong/Desktop/dataset/')
